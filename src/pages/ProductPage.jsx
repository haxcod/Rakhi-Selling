...
const ProductPage = () => {
 ...
 const fetchProducts = useCallback(async () => {
  ...
  const handleAddToCart = (newCart) => {
    ...
    const itemExists = existingCart?.find((item) => item.id === newCart.id);
    if (itemExists) {
      // If exists, update quantity
      const updatedCart = existingCart?.map((item) =>
        item.id === newCart.id ? { ...item, qty: item.qty + 1 } : item
      );
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        {selectedProduct && (
          ...
          // </div>
           ...
           <ProductDetails selectedProduct={selectedProduct} setShowProductDetails={()=>setSelectedProduct(null)}/>
